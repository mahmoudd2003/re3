...
def get_current_run_target():
    ...
    distribution = split_daily_count(
        daily_target,
        RUNS_PER_DAY
    )
...
def similarity_guard(old_text, new_text):
    ...
    if len(new_words) < max(
        5,
        int(len(old_words) * 0.70)
    ):
        return False, "Removed too much content"

    if len(new_words) > int(
        len(old_words) * 1.40
    ):
...
- التعديل يجب أن يكون طبيعيًا وخفيفًا.
- نسبة التعديل تقريبًا من 5% إلى 10%.
- يمكن إعادة ترتيب جملة قصيرة.
- يمكن استبدال كلمات بسيطة بمرادفات طبيعية.
- لا تستخدم أسلوبًا تسويقيًا.
- لا تجعل النص يبدو مكتوبًا بالذكاء الاصطناعي.
- أعد كتابة الفقرة فقط.
- بدون شرح.
- بدون علامات اقتباس.
...
def main():
    ...
    for post in posts:
        ...
        try:
            ...
            new_text, rewrite_message = (
                micro_rewrite_paragraph(
                    old_text
                )
            )
