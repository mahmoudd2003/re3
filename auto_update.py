...
def similarity_guard(old_text, new_text):
    ...
    if len(new_words) < max(5, int(len(old_words) * 0.85)):
        return False, "New text removed too much content"

    if len(new_words) > int(len(old_words) * 1.15):
        return False, "New text added too much content"
...
def micro_rewrite_paragraph(text):
    prompt = f"""
قم بتعديل طفيف جدًا على الفقرة التالية.
...
- لا تغيّر الأرقام أو الأسعار أو أوقات العمل إن وجدت.
- لا تغيّر الكلمات المفتاحية المهمة.
- التعديل يجب أن يكون خفيفًا وطبيعيًا، ولا يتجاوز 7% من النص.
- يمكن تحسين صياغة جملة قصيرة أو استبدال بعض الكلمات البسيطة دون تغيير المعنى.
- لا تستخدم أسلوبًا تسويقيًا.
- لا تجعل النص يبدو مكتوبًا بالذكاء الاصطناعي.
- أعد الفقرة فقط بدون شرح وبدون علامات اقتباس.
...
def add_row(rows, now, daily_target, run_target, distribution, post, status, old_paragraph="", new_paragraph="", message=""):
    rows.append({
        "run_time_utc": now,
        "run_index": RUN_INDEX,
    ...
    for post in posts:
        ...
        try:
            ...
            if not valid_paragraphs:
                add_row(rows, now, daily_target, run_target, distribution, post, "skipped", message="No valid paragraph found")
                continue
            ...
            if old_text == new_text:
                add_row(rows, now, daily_target, run_target, distribution, post, "skipped", old_text, new_text, rewrite_message)
                continue
            ...
            if response.status_code in [200, 201]:
                ...
                updated_count += 1
                add_row(rows, now, daily_target, run_target, distribution, post, "updated", old_text, new_text, rewrite_message)
            else:
                add_row(rows, now, daily_target, run_target, distribution, post, "failed", old_text, new_text, f"{response.status_code}: {response.text[:300]}")

        except Exception as e:
            add_row(rows, now, daily_target, run_target, distribution, post, "failed", message=str(e))
